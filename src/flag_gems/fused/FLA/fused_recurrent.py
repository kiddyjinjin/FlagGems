...
def fused_recurrent_gated_delta_rule_fwd_sp_for_qwen3_next_kernel(
    ...
    g,  # [1, T, 32/tp]
    beta,  # [1, T, 32/tp]
    o,  # 同 v， 是输出 [1, T, 32/tp, 128]
    h0,  # initial_state  # [4589, 8, 128, 128]
    ht,  # final_state，and since INPLACE_FINAL_STATE is True, we write to initial_state directly
    cu_seqlens,  # cu_seqlens = range(0, T)
    ssm_state_indices,  # zeros tensors of shape [T]
    num_accepted_tokens,  # None
    scale,  # 0.08838834764831845
    N: tl.int64,  # num of sequences   # should be 1
    T: tl.int64,  # num of tokens      # should be T, a dynamic value
    stride_q_b: tl.int64,
    stride_k_b: tl.int64,
    stride_v_b: tl.int64,
    stride_q_t: tl.int64,
    ...
    USE_QK_L2NORM_IN_KERNEL: tl.constexpr,
    IS_VARLEN: tl.constexpr,  # yes, it‘s variable length
    IS_CONTINUOUS_BATCHING: tl.constexpr,
    IS_SPEC_DECODING: tl.constexpr,
):
    i_k, i_v, i_nh = tl.program_id(0), tl.program_id(1), tl.program_id(2)
